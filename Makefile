...
#	eval "sed -i 's/app_path = os.path.abspath(os.getcwd())/app_path = \"\/usr\/bin\/bianbu-smart-album-${BACKEND}\"/g'" $(shell find $(DESTDIR)/usr/bin/bianbu-smart-album-${BACKEND} -name "*.py")
...
install: clean all
	...
	cp -rdf LICENSE $(DESTDIR)/usr/bin/bianbu-smart-album-${BACKEND}
	cp -rdf main.py $(DESTDIR)/usr/bin/bianbu-smart-album-${BACKEND}
	cp -rdf VERSION_NUMBER $(DESTDIR)/usr/bin/bianbu-smart-album-${BACKEND}
	make install_post
